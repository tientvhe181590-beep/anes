...
export function useOnboarding() {
        ...
        case 'availability':
          if (!data.trainingDaysPerWeek)
            errs.push({
              ...
              message: 'Please select training days per week',
            });
      ...
      if (error instanceof AxiosError) {
        setServerError(
          error.response?.data?.message ??
            'Unable to complete setup. Please check your connection and try again.',
        );
      } else {
  ...
  const goNext = useCallback(() => {
    ...
    if (currentStepIndex < steps.length - 1) {
      ...
      const allergiesOnly = data.allergies.filter((a) => !dietKeywords.includes(a) && a !== 'None');
      ...
      const request: OnboardingRequest = {
        ...
        goal: data.goal!,
        targetWeightKg:
          data.goal === 'WeightLoss' || data.goal === 'WeightGain' ? data.targetWeightKg : null,
        targetMuscleGroups: data.goal === 'MuscleGain' ? data.targetMuscleGroups : null,
